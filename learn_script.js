...
/////////////// 2.8 Callback ///////////////////////
//////////////////////////////////////////////////
// Callback-функция, или просто колбэк, — это функция, 
// переданная в другую функцию в качестве параметра.
...
/*
В примере выше мы передавали в колбэк функцию, объявленную через ключевое слово function, 
но механика колбэков работает с любыми функциями. Колбэк-функции, как правило, не нужно переиспользовать, 
поэтому чаще всего в коде вы встретите колбэки, созданные с помощью анонимных стрелочных функций.
*/
...
/* 
Функция, которая вызывает колбэк, может передать в вызов колбэка аргументы так же, 
как если бы она вызывала любую другую функцию:
*/
function functionWithCallback(callback) {
...
//Вот код, написанный без использования колбэков:
...
//setTimeout
setTimeout
//Эта функция позволяет выполнить колбэк через указанный промежуток времени один раз. 
//Разберем синтаксис метода на примере:
function showAd() {
...
/* 
В этом коде функция showAd, которую мы хотим запустить спустя 60 секунд после запуска
программы. Теперь обратим внимание на параметры, с которыми мы вызываем setTimeout.

Первый параметр — это колбэк-функция, которую мы хотим выполнить спустя какой-то промежуток времени.
Второй параметр указывает, через какой промежуток времени должна выполниться функция. 
Время указывается в миллисекундах, спустя которые мы хотим выполнить 
переданную функцию (прим. 1 сек = 1000 мс).
*/

//ОТМЕНА ЧЕРЕЗ CLEARTIMEOUT
/*
setTimeout возвращает «идентификатор таймера» (уникальное значение) 
timerId', который можно использовать для отмены дальнейшего выполнения.
*/
...
/*
Например, у нас салон по стрижке животных. 
При записи на услугу по стрижке наша система по умолчанию будет напоминать всем 
гостям о записи за 6 часов. Но клиент может написать и попросить не напоминать 
ему об этом, тогда нам нужно отменить ее. В коде ниже мы сначала планируем вызов 
функции и затем отменяем его. В результате ничего не происходит:
*/
...
//МЕТОД setInterval
/*
Метод setInterval имеет такой же синтаксис, как setTimeout, но отличие в том, 
...
/*
Например, мы делаем приложение-метроном для музыкантов. 
Чтобы реализовать такой функционал в коде, нам как раз и пригодится метод 
setInterval.
*/
...
/*
В большинстве браузеров, включая Chrome и Firefox, внутренний счетчик продолжает 
тикать во время показа
*/
alert / confirm / prompt
/*
Так что если вы запустите код выше и подождете с закрытием 
alert несколько секунд, то следующий alert будет показан сразу, как только вы 
закроете предыдущий. Интервал времени между сообщениями alert будет короче, 
чем 2 секунды.
*/

Два способа запускать что - то регулярно
Один из них — setInterval. 
Другим является вложенный setTimeout
...
Вложенный setTimeout — более гибкий метод, чем setInterval.С его помощью 
последующий вызов может быть задан по - разному в зависимости от результатов
предыдущего.Например, необходимо написать сервис, который отправляет запрос 
